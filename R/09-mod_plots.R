    ...
    bslib::layout_columns(
      ...
      shiny::div(
        ...
        bslib::card(
          ...
          bslib::card_header("Read AquaCrop output"),
          shiny::p(class = "step-hint",
            "Choose a ", shiny::tags$code(".OUT"), " log from the project (usually under ",
            shiny::tags$code("OUTP/"), ") after a simulation."
          ),
          shiny::selectInput(ns("dlf"), "Output file (.OUT)", choices = character()),
          shiny::div(
    ...
    shiny::observeEvent(input$read, {
      tryCatch({
        ...
        if (is.null(path) || !nzchar(path) || !file.exists(path))
          stop("Choose an existing .OUT file.")
