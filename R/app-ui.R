aquacropr_studio_theme <- function() {
  bslib::bs_theme(
    version = 5,
    bootswatch = "flatly",
    primary = "#1b5e40",
...
aquacropr_studio_css_href <- function() {
  css_file <- system.file("www", "app.css", package = "aquacropr.studio")
  stamp <- tryCatch(as.integer(file.info(css_file)$mtime), error = function(e) 1L)
  paste0("aquacroprshiny/app.css?v=", stamp)
}

aquacropr_studio_ui <- function() {
  shiny::addResourcePath(
    "aquacroprshiny",
    system.file("www", package = "aquacropr.studio", mustWork = TRUE)
  )

  bslib::page_fillable(
    theme = aquacropr_studio_theme(),
    padding = 0,
    gap = 0,
    fillable_mobile = TRUE,
    shiny::tags$head(
      shiny::tags$link(rel = "stylesheet", type = "text/css", href = aquacropr_studio_css_href()),
      shiny::tags$script(shiny::HTML("
      Shiny.addCustomMessageHandler('ac-nav', function(value) {
      ...
      Shiny.addCustomMessageHandler('ac-lock-nav', function(msg) {
        var rail = document.querySelector('.app-nav-rail');
        if (!rail) return;
        rail.classList.toggle('nav-locked', !!(msg && msg.locked));
        rail.setAttribute('aria-busy', (msg && msg.locked) ? 'true' : 'false');
      });
      Shiny.addCustomMessageHandler('ac-append-text', function(msg) {
      ...
      Shiny.addCustomMessageHandler('ac-set-progress', function(msg) {
    ...
    shiny::div(
      ...
      shiny::span(
        ...
        shiny::strong("aquacropr"),
        shiny::span(" studio")
      ),
      shiny::span(class = "app-topbar-meta", "AquaCrop from R")
    ),
      ...
      bslib::layout_sidebar(
        ...
        sidebar = bslib::sidebar(
          ...
          shiny::p(
            ...
            " in ", shiny::tags$code("aquacropr-studio-sessions"),
            " under your home folder, so they show up without a project directory. Older copies in a project folder are still listed. Paths and loaded tables come back; AquaCrop output is re-read from ",
            shiny::tags$code(".OUT"), " files on disk."
          ),
