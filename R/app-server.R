aquacropr_studio_server <- function(input, output, session) {
  ...
  project <- do.call(shiny::reactiveValues, c(st, list(session_reset = 0L, session_load = 0L)))
  ...
  shiny::observeEvent(input$session_init, {
    if (isTRUE(project$run_busy)) {
      shiny::showNotification("Wait until the current AquaCrop run finishes.", type = "warning")
      return()
    }
    ...
    session$sendCustomMessage("ac-nav", "project")
  ...
  output$session_status <- shiny::renderUI({
    ...
    exe_ok <- aquacrop_launcher_ok(project)
    dir_ok <- nzchar(project$project_dir) && dir.exists(project$project_dir)
    plugin <- plugin_dir_of(project)
    plugin_ok <- nzchar(plugin) && dir.exists(plugin)
    simul_ok <- nzchar(simul_dir_of(project)) && dir.exists(simul_dir_of(project))
    ac_label <- if (identical(project$aquacrop_launch %||% "exe", "cmd")) {
      paste("cmd:", basename(exe %||% ""))
    } else {
      basename(exe %||% "")
    }
    ...
    shiny::div(
      ...
      block(
        "Plugin folder",
        shiny::div(class = "session-value", if (nzchar(plugin)) plugin else "\u2014"),
        shiny::div(
          class = "session-row",
          shiny::span(class = "session-value", ac_label),
          status_pill(plugin_ok && exe_ok, "Ready", "Missing")
        ),
        shiny::div(
          class = "session-row",
          shiny::span(class = "session-value", "SIMUL/"),
          status_pill(simul_ok, "Ready", "Missing")
        )
      ),
      block(
        "Project",
      ...
      block(
        "List project",
        shiny::div(class = "session-value", project$list_project %||% "\u2014")
      ),
      block(
        "Parameters",
