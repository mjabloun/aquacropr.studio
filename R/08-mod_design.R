  ...
  bslib::layout_columns(
    ...
    bslib::card(
      ...
      shiny::p(class = "step-hint",
        "Draw a training design over parameter bounds, run AquaCrop for every row, optionally score with the saved objective, and fit a Kriging metamodel."
      ),
      ...
      shiny::div(
        ...
        shiny::actionButton(ns("run"), "Run AquaCrop on design", class = "btn-primary")
      ),
      shiny::hr(),
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$run, {
      tryCatch({
        ...
        if (!nzchar(out_path)) stop("Run one simulation first so the output .OUT path is known.")
        ...
        log_append(project, "Running param design (", n, " AquaCrop jobs).")
    ...
    shiny::observeEvent(input$score, {
      tryCatch({
        ...
        if (is.null(project$design_run)) stop("Run the design through AquaCrop first.")
