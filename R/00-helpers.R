...
plugin_exe_basenames <- function() {
  c(
    "AquaCrop73.exe", "aquacrop.exe", "aquacrop",
    "ACsaV73.exe", "ACsaV72.exe", "ACsaV71.exe"
  )
}

find_exe_in_plugin <- function(plugin) {
  plugin <- trimws(plugin %||% "")
  if (!nzchar(plugin) || !dir.exists(plugin)) return("")
  for (nm in plugin_exe_basenames()) {
    p <- file.path(plugin, nm)
    if (file.exists(p))
...
default_plugin_dir <- function() {
  exe <- tryCatch(get_aquacrop_path(), error = function(e) "")
  if (nzchar(exe) && file.exists(exe))
    return(dirname(normalizePath(exe, winslash = "/", mustWork = FALSE)))
  fallback <- "C:/JHI/Projects/Daisy/R-DAISY_v2/AquaCrop73_windows"
  if (dir.exists(fallback))
    return(normalizePath(fallback, winslash = "/", mustWork = FALSE))
  ""
}

default_aquacrop_exe <- function() {
  hit <- find_exe_in_plugin(default_plugin_dir())
  if (nzchar(hit)) return(hit)
  tryCatch(get_aquacrop_path(), error = function(e) "")
}

plugin_dir_of <- function(project) {
  pd <- project$plugin_dir %||% ""
  if (nzchar(pd)) return(pd)
  ""
}

aquacrop_exe_of <- function(project) {
  hit <- find_exe_in_plugin(plugin_dir_of(project))
  if (nzchar(hit)) return(hit)
  project$aquacrop_exe %||% ""
}

simul_dir_of <- function(project) {
  plugin <- plugin_dir_of(project)
  sib <- if (nzchar(plugin)) file.path(plugin, "SIMUL") else ""
  if (nzchar(sib) && dir.exists(sib))
    return(normalizePath(sib, winslash = "/", mustWork = FALSE))
  project$simul_dir %||% ""
}

sync_plugin_paths <- function(project) {
  plugin <- plugin_dir_of(project)
  project$aquacrop_exe <- aquacrop_exe_of(project)
  project$simul_dir <- simul_dir_of(project)
  invisible(plugin)
}

aquacrop_cmd_of <- function(project) {
  if (!identical(project$aquacrop_launch %||% "exe", "cmd")) return(NULL)
  cmd <- trimws(project$aquacrop_cmd %||% "")
  if (!nzchar(cmd)) return(NULL)
  if (!grepl("{aquacrop_exe}", cmd, fixed = TRUE) &&
      !grepl("{working_dir}", cmd, fixed = TRUE)) {
    stop("Command template must include {aquacrop_exe} and/or {working_dir}.")
  }
  cmd
}

aquacrop_launcher_ok <- function(project) {
  exe <- aquacrop_exe_of(project)
  exe_ok <- nzchar(exe) && file.exists(exe)
  if (identical(project$aquacrop_launch %||% "exe", "cmd")) {
    cmd <- trimws(project$aquacrop_cmd %||% "")
    return(exe_ok && nzchar(cmd) && (
...
output_dir_of <- function(project) {
  od <- project$output_dir %||% ""
  if (nzchar(od)) return(od)
  project$project_dir %||% ""
}

example_wheat_dir <- function() {
  cands <- c(
    file.path(getwd(), "Example_aquacrop"),
    file.path(dirname(getwd()), "Example_aquacrop"),
    "C:/JHI/Projects/Daisy/R-DAISY_v2/Example_aquacrop"
  )
  for (d in cands) {
...
shiny_ac_progress <- function(i, n, label = "AquaCrop run") {
...
send_ac_progress <- function(session, bar_id, lab_id, i, n, label = "AquaCrop run",
                             detail = NULL) {
  n <- max(as.integer(n), 1L)
  i <- max(as.integer(i), 0L)
  pct <- 100 * (i / n)
  ...
  session$sendCustomMessage("ac-set-progress", list(
    id = bar_id, label_id = lab_id, pct = pct, label = lab, hide = FALSE
  ))
}

hide_ac_progress <- function(session, bar_id) {
  session$sendCustomMessage("ac-set-progress", list(id = bar_id, hide = TRUE))
}
...
with_nav_lock <- function(session, project, fun) {
  ...
  session$sendCustomMessage("ac-lock-nav", list(locked = TRUE))
    ...
    shiny::isolate({
      tryCatch(
        fun(),
          ...
          session$sendCustomMessage("ac-lock-nav", list(locked = FALSE))
        }
      )
    })
  }, once = TRUE)
...
empty_project_state <- function() {
  list(
    aquacrop_exe = default_aquacrop_exe(),
    aquacrop_cmd = "",
    aquacrop_launch = "exe",
    ...
    output_dir = "",
    simul_dir = {
      plugin <- default_plugin_dir()
      sib <- if (nzchar(plugin)) file.path(plugin, "SIMUL") else ""
      if (nzchar(sib) && dir.exists(sib))
        normalizePath(sib, winslash = "/", mustWork = FALSE)
      else ""
    },
    list_project = "",
    config_path = "",
...
studio_session_home_dir <- function() {
  ...
  file.path(home, "aquacropr-studio-sessions")
}
...
studio_session_search_dirs <- function(project = NULL) {
  ...
  if (nzchar(pd) && dir.exists(pd))
    dirs <- c(dirs, file.path(pd, "aquacropr-studio-sessions"))
  wd <- getwd()
  if (nzchar(wd))
    dirs <- c(dirs, file.path(wd, "aquacropr-studio-sessions"))
...
write_studio_session <- function(path, state, label = "") {
  obj <- list(
    format = "aquacropr.studio.session",
    version = 1L,
    saved_at = Sys.time(),
...
read_studio_session <- function(path) {
  ...
  obj <- readRDS(path)
  if (!is.list(obj) || !identical(obj$format, "aquacropr.studio.session"))
    stop("Not an aquacropr studio session file: ", path)
  st <- obj$state
...
apply_project_state <- function(project, st) {
  ...
  project$run_busy <- FALSE
  if (!nzchar(project$plugin_dir %||% "") &&
      nzchar(project$aquacrop_exe %||% "") && file.exists(project$aquacrop_exe)) {
    project$plugin_dir <- dirname(normalizePath(project$aquacrop_exe, winslash = "/", mustWork = FALSE))
  }
...
need_project <- function(project) {
  pd <- plugin_dir_of(project)
  if (!nzchar(pd) || !dir.exists(pd))
    stop("Set an existing plugin directory on the Project page (folder with the executable and SIMUL/).")
  if (!aquacrop_launcher_ok(project))
    stop("No AquaCrop executable found in the plugin directory (and a command template if you chose that launch mode).")
...
need_objective <- function(project) {
  if (is.null(project$objective) || !inherits(project$objective, "aquacropr_objective"))
...
obs_set_to_spec <- function(set) {
  ...
  sim <- set$sim_file %||% ""
  if (!nzchar(sim))
    stop("Choose a simulated .OUT for '", tag, "'.")
  objective_spec(
    obs_data = set$data,
    value_map = vm,
    join_cols = join,
...
resolve_objective_sim_file <- function(spec, project) {
  comps <- if (inherits(spec, "aquacropr_composite_objective")) spec$components else list(spec)
  files <- list_out_files(project)
  for (comp in comps) {
    f <- comp$output_file %||% ""
    if (!nzchar(f)) next
...
objective_component_list <- function(spec) {
  if (inherits(spec, "aquacropr_composite_objective")) return(spec$components)
  nm <- spec$name %||% "obs"
  stats::setNames(list(spec), nm)
}

resolve_component_out <- function(comp, project) {
...
objective_fit_series <- function(spec, project) {
  ...
  for (nm in names(comps)) {
    comp <- comps[[nm]]
    path <- resolve_component_out(comp, project)
    if (!nzchar(path) || !file.exists(path))
      stop("No .OUT found for '", nm, "'. Calibrate or run AquaCrop first.")
...
write_list_projects <- function(project) {
  stem <- trimws(project$list_project %||% "")
  if (!nzchar(stem)) return(invisible(FALSE))
  plugin <- plugin_dir_of(project)
  if (!nzchar(plugin) || !dir.exists(plugin)) return(invisible(FALSE))
  list_dir <- file.path(plugin, "LIST")
  dir.create(list_dir, recursive = TRUE, showWarnings = FALSE)
  if (!grepl("\\.(PRM|PRO)$", stem, ignore.case = TRUE))
    stem <- paste0(stem, ".PRM")
  writeLines(stem, file.path(list_dir, "ListProjects.txt"))
  invisible(TRUE)
}

run_project_aquacrop <- function(project, values = NULL, show_log = TRUE) {
  ...
  if (!is.null(values)) {
    ...
    render_templates(
      ...
      output_dir = output_dir_of(project)
    )
  }
  write_list_projects(project)
  sd <- simul_dir_of(project)
  if (nzchar(sd) && dir.exists(sd)) {
    tryCatch(set_simul_dir(sd), error = function(e) NULL)
  }
  status <- run_aquacrop(
    working_dir = plugin,
    aquacrop_exe = aquacrop_exe_of(project),
    show_log = show_log,
    cmd = aquacrop_cmd_of(project),
  ...
  outs <- list_out_files(project)
  if (length(outs)) {
    info <- file.info(outs)
    project$last_sim_path <- outs[[which.max(info$mtime)]]
  }
  project$last_status
}

describe_last_run <- function(project) {
  plugin <- plugin_dir_of(project)
  lines <- c(
    paste0("plugin = ", plugin),
    paste0("status = ", project$last_status %||% "—"),
    paste0("ListProjects = ", project$list_project %||% "(unchanged)")
  )
  outp <- file.path(plugin, "OUTP")
  if (dir.exists(outp)) {
    files <- list.files(outp, pattern = "\\.(OUT|out)$")
    drop <- tolower(tools::file_path_sans_ext(files)) %in%
      c("alldone", "listprojectsloaded")
    files <- files[!drop]
    lines <- c(lines, "", "OUTP/", if (length(files)) paste0("  ", files) else "  (empty)")
  } else {
    lines <- c(lines, "", "OUTP/ not found under the plugin folder.")
  }
...
parse_and_validate_yaml <- function(yaml_text, template_dir) {
  ...
  config <- read_parameters(tmp)
  if (nzchar(template_dir) && dir.exists(template_dir))
    validate_parameters(config, template_dir = template_dir)
  config
}
...
params_to_df <- function(config) {
  ...
  as.data.frame(config$parameters[, c("name", "default", "min", "max", "from_file", "to_file")],
                stringsAsFactors = FALSE)
}
...
tables_to_param_config_yaml <- function(params_df, curves_df = NULL) {
  ...
  yaml::as.yaml(list(parameters = parameters), indent = 2)
}

find_ac_placeholders <- function(dir) {
  ...
  files <- files[!grepl("\\.(ya?ml|rds|csv|png|md|R)$", files, ignore.case = TRUE)]
...
list_out_files <- function(project) {
  roots <- character()
  pd <- project$project_dir %||% ""
  plugin <- plugin_dir_of(project)
  if (nzchar(pd) && dir.exists(pd)) {
    roots <- c(roots, file.path(pd, "OUTP"), file.path(pd, "output"), pd)
  }
  if (nzchar(plugin) && dir.exists(plugin))
    roots <- c(roots, file.path(plugin, "OUTP"))
  roots <- unique(roots)
  files <- unlist(lapply(roots, function(d) {
    if (!dir.exists(d)) return(character())
    list.files(d, pattern = "\\.(OUT|out)$", full.names = TRUE, recursive = TRUE)
  }), use.names = FALSE)
  files <- unique(normalizePath(files, winslash = "/", mustWork = FALSE))
  drop <- tolower(tools::file_path_sans_ext(basename(files))) %in%
    c("alldone", "listprojectsloaded")
  files[!drop]
}
...
make_sim_reader <- function(daily = FALSE, groups = NULL, fun = NULL) {
  ...
  function(path, ..., data = NULL) {
    if (!is.null(data)) return(process(data))
    if (is.data.frame(path)) return(process(path))
    process(read_out(path))
  }
}
...
plot_series <- function(dt, y_cols, x_col = "Date", geom = "line",
  ...
  p +
    ggplot2::labs(title = title, x = x_col, y = NULL, colour = NULL, fill = NULL) +
    ggplot2::theme_minimal(base_size = 13) +
    ggplot2::theme(legend.position = "bottom")
}
...
rel_output_path <- function(project, abs_path) {
  ...
  for (root in c(wd, plugin)) {
    if (!nzchar(root)) next
    wd_n <- normalizePath(root, winslash = "/", mustWork = FALSE)
    if (startsWith(tolower(p_n), tolower(wd_n))) {
      return(substring(p_n, nchar(wd_n) + 2L))
    }
  }
  abs_path
}
...
ac_editor_ui <- function(id, placeholder = "", height = "520px") {
...
update_ac_editor <- function(session, editorId, value) {
