mod_obs_set_ui <- function(id) {
  ...
  shiny::div(
    ...
    section(
      "4", "Match to an AquaCrop output",
      "Join this table to one simulation log, then choose the goodness-of-fit metric.",
      bslib::layout_columns(
        col_widths = c(5, 4, 3),
        shiny::selectInput(ns("sim_file"), "Simulated .OUT", choices = character()),
    ...
    section(
      "5", "Map observed columns to simulated columns",
          "Pick columns from the loaded table and the .OUT, then add a pair. Label and weight are optional.",
      bslib::layout_columns(
  ...
  bslib::layout_columns(
    ...
    bslib::card(
      ...
      shiny::p(class = "step-hint",
        ...
        "It is a weighted mean of one or more observation tabs (different files, joins, metrics, and .OUT logs)."
      ),
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$score, {
      tryCatch({
        ...
        if (!nzchar(path) || !file.exists(path))
          stop("Choose a simulated .OUT on the observation tab (or run AquaCrop so OUTP/ has a file).")
        res <- evaluate_objective(spec, path, return_per_pair = TRUE, plot = TRUE)
        project$last_score <- res
        last_plot(res$plot)
        metric_lab <- {
          if (inherits(spec, "aquacropr_composite_objective")) {
