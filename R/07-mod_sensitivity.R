  ...
  bslib::layout_columns(
    ...
    bslib::card(
      ...
      shiny::p(class = "step-hint",
        "Choose a test, then AquaCrop is run for every row of that design. Response is the saved objective score, or the mean of one output column."
      ),
      shiny::radioButtons(
        ns("method"),
        "Which test to run",
      ...
      shiny::conditionalPanel(
        ...
        shiny::p(class = "step-hint",
          "Sobol-Jansen needs about N \u00d7 (k + 2) AquaCrop runs for k parameters. Keep N small for a trial."
        )
      ),
      ...
      shiny::conditionalPanel(
        ...
        ns = ns,
        shiny::textInput(ns("resp_col"), "Simulated column", value = "CC")
      ),
    ...
    shiny::tagList(
      bslib::card(
        ...
        shiny::p(class = "step-hint",
          "Select rows to calibrate. Suggested names have \u03bc* or ST at least 10% of the maximum. Then send them to Calibrate."
        ),
        shiny::div(
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$go, {
      tryCatch({
        ...
        if (!length(files) && is.null(project$last_sim_path))
          stop("Run one simulation first so the output .OUT path is known.")
        ...
        log_append(project, "SA design ", method, " with ", n_runs, " AquaCrop runs.")
        shiny::showNotification(paste("Running", n_runs, "AquaCrop evaluations."), type = "message")
          ...
          tryCatch({
            sa_run <- shiny::withProgress(message = paste("Sensitivity:", n_runs, "runs"), value = 0, {
              run_sa_design(
                ...
                progress = function(i, n) shiny_ac_progress(i, n, "AquaCrop run")
              )
            })
    ...
    shiny::observeEvent(input$use_for_cal, {
      tryCatch({
        ...
        project$calibrate_names <- expand_names(project$config, nms)
        log_append(
