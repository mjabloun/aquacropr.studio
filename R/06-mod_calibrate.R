        ...
        bslib::layout_columns(
          ...
          bslib::card(
            ...
            shiny::p(class = "step-hint",
              "Fits the selected parameters against the saved objective. Other parameters stay at their defaults. Each candidate is one AquaCrop run, so keep ",
            ...
            shiny::p(class = "step-hint",
              "Filled from Sensitivity when you use selected indices. Leave empty to fit every parameter."
            ),
          ...
          shiny::tagList(
            ...
            bslib::card(
              ...
              shiny::p(class = "step-hint",
                "One line per generation (NP AquaCrop runs). The progress bar also updates after each evaluation."
              ),
              shiny::div(
        ...
        bslib::layout_columns(
          ...
          bslib::card(
            ...
            shiny::p(class = "step-hint",
              ...
              shiny::tags$code(".OUT"), ". Simulated line is the latest AquaCrop output (including the automatic best-parameter run)."
            ),
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observe({
      ...
      nms <- if (!is.null(project$config))
        parameter_names(project$config) else character()
    ...
    shiny::observeEvent(input$go, {
      tryCatch({
        ...
        if (is.null(sim_file) || !file.exists(sim_file %||% "")) {
          ...
          if (!length(files)) stop("Run a simulation first so the objective knows which .OUT to read.")
          sim_file <- files[[1]]
        }
        nms_fit <- expand_names(project$config, input$param_names)
        if (!length(nms_fit))
        ...
        log_append(
          ...
          ", fitting ", length(nms_fit), " of ", length(parameter_names(project$config)),
          " names)."
        )
        shiny::showNotification("Calibration running \u2014 progress appears in DEoptim progress.", type = "message")
        session$sendCustomMessage("ac-append-text", list(
        ...
        send_ac_progress(session, session$ns("cal_bar"), session$ns("cal_bar_lab"),
                            0L, 1L, "eval")
        np <- as.integer(input$NP)
        itermax <- as.integer(input$itermax)
        with_nav_lock(session, project, function() {
          tryCatch({
            fit <- shiny::withProgress(message = "Calibrating (AquaCrop runs)", value = 0, {
          calibrate(
            ...
            objective = of$spec,
            sim_file = sim_file,
            working_dir = plugin_dir_of(project),
            ...
            reporter = function(info) {
              shiny_ac_progress(info$evals, info$n_total, "eval")
              send_ac_progress(
        ...
        hide_ac_progress(session, session$ns("cal_bar"))
        tryCatch({
          session$sendCustomMessage("ac-append-text", list(
            id = session$ns("de_trace"), reset = FALSE,
            line = "Running AquaCrop with best parameters..."
          ))
          shiny::withProgress(message = "Running best parameters", value = 0.4, {
            run_project_aquacrop(project, values = fit$best_params, show_log = TRUE)
          })
    ...
    fit_obs_plots <- shiny::reactive({
      ...
      tryCatch({
        ...
        if (!length(plots))
          stop("No series to plot. Check the objective mapping, join columns, and .OUT.")
        fit_obs_err("")
        plots
      }, error = function(e) {
    ...
    output$of_view_score <- shiny::renderText({
      ...
      if (!nzchar(path) || !file.exists(path))
        return("No .OUT found yet. Calibrate (or run AquaCrop) first.")
      res <- tryCatch(
